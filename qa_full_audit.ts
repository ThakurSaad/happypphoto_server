...
const blockers: string[] = ["", "## Blocker Log"];
...
function extractRoutes(appInst: any) {
  ...
  const indexContent = fs.readFileSync(
    path.join(process.cwd(), "src", "app", "routes", "index.ts"),
    "utf-8",
  );
  ...
  for (const mod of routeModules) {
    const importRegex = new RegExp(
      `import\\s+${mod.routeVar}\\s+from\\s+["']([^"']+)["']`,
    );
...
async function runTests() {
  try {
    ...
    const uniqueRoutes = Array.from(
      new Set(allRoutes.map((r) => `${r.method} ${r.path}`)),
    ).map((str) => {
    ...
    for (const route of uniqueRoutes) {
      ...
      if (isIntegrityOk) {
        passedRoutes++;
        report.push(
          `| ${route.method} | \`${route.path}\` | ${statusCode} | ✅ Valid |`,
        );
      } else {
        failedRoutes++;
        report.push(
          `| ${route.method} | \`${route.path}\` | ${statusCode} | ❌ Failed |`,
        );
        blockers.push(`### ${route.method} ${route.path}`);
        blockers.push(`**Error**: ${errorMsg}`);
        blockers.push(
          `**Test Context**: Fuzzing request with empty payload/anonymous access. Expected 401/400, got ${statusCode}.`,
        );
      }
    }
    ...
    report.splice(
      2,
      0,
      `**Coverage Percentage**: ${coverage}% (${testedRoutes}/${totalRoutes} routes tested)`,
    );
    report.splice(
      3,
      0,
      `**Results**: ${passedRoutes} Passed, ${failedRoutes} Failed (Server Errors/Malformed Responses)\n`,
    );
